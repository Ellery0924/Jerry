...
function _allHandler(type) {

    blockPointCount = 0;
    
    Logger.instances.forEach(function (ins) {

        type === 'continue' ?
            ins.blockPromise.resolve(ins.response) :
            ins.blockPromise.reject();
    });
}
...
function init() {
    ...
    wsServer.on('connection', function (socket) {
        ...
        socket
            ...
            .on('allBlockPointContinue', function () {

                _allHandler('continue');
            })
            .on('allBlockPointAbort', function () {
